...
## RUBRIC
	...
	# 5 It could be as simple as, for example, a new function that you write by hand to calculate the standard deviation
	# 6 Create and render a quarto document that includes at least:
			# The table, regression results, and figure, with appropriate captions (1 pt)
	# 7 Inline R code in at least 2 places, 1 pulling a statistic from a table (i.e., using gtsummary::inline_text()) and 1 printing something else (like we did with the mean age in the example) (1 pt)
	# 8 Cross-references to a table and a figure at least once each (1 pt)
	# 9 A brief description of the data, including its source (1 pt)
	# 10 Use the {here} package every time you refer to file paths when reading in data and saving any files (1 pt)
